...
beforeEach(async () => {
  tempRootDir = await mkdtemp(path.join(os.tmpdir(), 'omnicrawl-control-plane-outputs-'));
...
describe('control-plane structured output access', () => {
  it('loads previews and downloads for downloadable json outputs', async () => {
    ...
    expect(capture?.destinationId).toBe(IMPLICIT_DOWNLOADABLE_JSON_DESTINATION_ID);
    ...
    const preview = await getControlPlaneRunStructuredOutputPreview({
      ...
      destinationId: IMPLICIT_DOWNLOADABLE_JSON_DESTINATION_ID,
