...
function formatPipelineId(pipelineId: string): string {
  if (!pipelineId.includes('-')) {
    return pipelineId;
  }

  const suffix = pipelineId.split('-').at(-1);
  return suffix ? `...${suffix}` : pipelineId;
}
...
export function PipelineListClient({ pipelines }: { pipelines: ControlPlanePipeline[] }) {
  ...
  const copyPipelineId = async (pipelineId: string) => {
    try {
      ...
      setCopiedPipelineId(pipelineId);
      ...
      copyFeedbackTimer.current = window.setTimeout(() => {
        setCopiedPipelineId((current) => (current === pipelineId ? null : current));
      }, 2000);
    } catch {
      setCopiedPipelineId(null);
    }
  };

  return (
    <TooltipProvider delayDuration={150}>
      <div className="grid min-w-0 w-full gap-4 overflow-hidden">
            ...
            <div className="hidden overflow-hidden rounded-sm border border-border md:block">
              <div className="w-full overflow-x-auto">
                <Table>
                  <TableHeader>
                    <TableRow>
                      <TableHead className="min-w-0 w-[1%]">Name</TableHead>
                  ...
                  <TableBody>
                    {pipelines.map((pipeline) => (
                      <TableRow key={pipeline.pipelineId}>
                        <TableCell className="min-w-0 max-w-[360px]">
                          <div className="flex min-w-0 flex-col">
                            <span
                              className="max-w-[200px] truncate font-semibold text-[var(--theme-text-primary)] sm:max-w-[300px]"
                              title={pipeline.name}
                            >
                              {pipeline.name}
                            </span>
                            <div className="mt-0.5 flex items-center gap-2">
                              <Tooltip>
                                <TooltipTrigger asChild>
                                  <span className="cursor-default font-mono text-xs text-[var(--theme-text-secondary)]">
                                    {formatPipelineId(pipeline.pipelineId)}
                                  </span>
                                </TooltipTrigger>
                                <TooltipContent>
                                  <p className="font-mono text-xs">{pipeline.pipelineId}</p>
                                </TooltipContent>
                              </Tooltip>
                              <button
                                type="button"
                                aria-label="Copy Pipeline ID"
                                className="inline-flex items-center justify-center text-[var(--theme-text-secondary)] transition-colors hover:text-[var(--theme-text-primary)]"
                                onClick={() => void copyPipelineId(pipeline.pipelineId)}
                              >
                                {copiedPipelineId === pipeline.pipelineId ? (
